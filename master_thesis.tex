...
\chapter{Materials and Methods} \label{chap:methods}% etat de l'art
...
\footnotetext{\url{https://commons.wikimedia.org/wiki/User:Shiyu\_ Ji\#/media/File:Kernel\_ trick\_ idea.svg}}
...
\section{The CIFAR10 dataset} \label{sec:CIFAR10}
...
\begin{tabular}{|c|c|c|}
  ...
  \hline
\end{tabular}
\captionof{table}{Experiment 6 results table.}  
\end{center}
...
\begin{tabular}{|c|c|c|}
  ...
  \hline
\end{tabular}
\captionof{table}{Summary table of the \textbf{best} results obtained for the experiments on CIFAR-10 dataset. These are corresponding to the following parameters : (Exp 1) betweeen 10 and 40 epochs ; (Exp 2) ADAGRAD ; (Exp 3) dropout=0.2, dropout=0 ; (Exp 4) weight decay = 0.001 ; (Exp 5) data aug = yes, (Exp 6) noise=0.1, noise=0. The result highlighted in \textcolor{ForestGreen}{green} is the best result obtained. The result highlighted in \textcolor{red}{red} is the worst result obtained.}
\label{table:summary-cifar-10}

\begin{figure}[H]
  \centering
  \includegraphics[scale=0.45]{Code/ch5-ModelPrototype/figures_result/bar-graph_cifar10_res.png}
...
\textcolor{BrickRed}{\danger  \textbf{The detailed results of the experiments for ALL-IDB1 dataset are available in the appendix \autoref{sec:appendix-case-A}.}} It contains detailed graphics, table and description of the results of each experiment done on \textbf{ALL-IDB1 dataset}. Each parameter is approached one by one and interpreted concerning their efficiency and the impact on the model's accuracy. Therefore, note that this current section is a \textbf{summary} of all these interpretations on obtained results for the first dataset ALL-IDB1.  
\end{minipage}
}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.3]{Code/ch6-LeukemiaSubtypes/figures_result/ALL_IDB1/all_idb1-best_worst}
...
\fbox{ %fbox est utilisé pour voir les bords de la minipage
\begin{minipage}[c]{14.5cm}
\textcolor{BrickRed}{\danger  \textbf{The detailed results of the experiments for ALL-IDB2 dataset are available in the appendix \autoref{sec:appendix-case-B}.}} It contains detailed graphics, table and description of the results of each experiment done on \textbf{ALL-IDB2 dataset}. Each parameter is approached one by one and interpreted concerning their efficiency and the impact on the model's accuracy. Therefore, note that this current section is a \textbf{summary} of all these interpretations on obtained results for the second dataset ALL-IDB2.  
\end{minipage}
}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.27]{Code/ch6-LeukemiaSubtypes/figures_result/ALL_IDB2/all_idb2-best_worst.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.5]{Code/ch6-LeukemiaSubtypes/figures_result/ALL_IDB2/all_idb2-summary.png}
...
\fbox{ %fbox est utilisé pour voir les bords de la minipage
\begin{minipage}[c]{14.5cm}
\textcolor{BrickRed}{\danger  \textbf{The detailed results of the experiments for ALL-IDB and DEMIR-LEUKEMIA datasets are available in the appendix \autoref{sec:appendix-case-C}.}} It contains detailed graphics, table and description of the results of each experiment done on \textbf{ALL-IDB and DEMIR LEUKEMIA datasets}. Each parameter is approached one by one and interpreted concerning their efficiency and the impact on the model's accuracy. Therefore, note that this current section is a \textbf{summary} of all these interpretations on obtained results for the datasets ALL-IDB1 and DEMIR-LEUKEMIA.  
\end{minipage}
}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.27]{Code/ch6-LeukemiaSubtypes/figures_result/LEUK-SUBTYPES/leukSub-best_worst.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.5]{Code/ch6-LeukemiaSubtypes/figures_result/LEUK-SUBTYPES/leukSub-summary.png}
...
% Introduction: parler que pour l'instant image classification est seulement utilisée en tant que computer-aided mais qu'il faut l'avis d'un expert -> on va utiliser ça pour améliorer notre modèle 
% http://burrsettles.com/pub/settles.activelearning.pdf 
% http://proceedings.mlr.press/v16/settles11a/settles11a.pdf
% https://e-archivo.uc3m.es/bitstream/handle/10016/28068/visually_ijgi_2018.pdf?sequence=1&isAllowed=y
...
% How does Active Learning work? https://www.kdnuggets.com/2018/10/introduction-active-learning.html 
% https://www.datacamp.com/community/tutorials/active-learning --> Putting it all together section 
% https://towardsdatascience.com/active-learning-tutorial-57c3398e34d = pseudo-code
...
%Label+attribute elicitation for image annotation? https://tel.archives-ouvertes.fr/tel-00752022v2/document section 4.5
...
% Entropy sampling + least confident + margin sampling 
...
% un mot sur expected error reduction
...
% http://www.cs.cmu.edu/~tom/10709_fall09/readings/CVPR08.pdf 
%  https://www.kdnuggets.com/2018/10/introduction-active-learning.html : performance section
...
Moreover, we have related active learning to semi-supervised learning which is a category of algorithms that is \textbf{vulnerable to biases} since it is based on assumptions which are in fact not always verified\footnote{cfr. \autoref{sec:SSL}}. % http://www.cs.toronto.edu/~tl/papers/lumastersthesis.pdf 
% Unless the learner is absolutely sure of an assumption that holds on the relationship between labels and the unlabeled data structure (if no assumptions are made then we refer to it as the no-prior-knowledge setting) then one cannot hope to obtain a significant advantage in the sample complexity1 of semi-supervised learning over that of supervised learning
...
%Recapitulation du problème (chap 1-4) 
...
%Contribution (chap 5-6): paramètres du modèle sur data normale et médicale -> résultats ? 
...
%Limitations (chap 7): comment améliorer ses résultats -> active learning 
...
% https://stats.stackexchange.com/questions/378338/how-can-i-mix-image-and-data-into-a-cnn

\subsection{Multiple inputs and mixed data}
% dans ce mémoire on a focus sur l'image mais image = une variable parmi tant d'autres
% parler des différentes variables qui peuvent être utilisées (numeric, categorical, ...) 
...
% expliquer le concept de multiple inputs & mixed data (définir ces termes)  
...
% => étapes load, pre-process, prepare 
% expliquer pourquoi ça marcherait => pseudo-code qui résume le model 
...
% conclure la section en ouvrant le sujet sur différents travaux https://www.hindawi.com/journals/jece/2017/9240407/ mais pas grand chose donc ça peut être du future work 
